...
def basic_crud_operation(request):
    # Create first type
    data = {"title": "Movie", "featured_product": Product(pk=1)}
    col = Collection(**data)
    col.save()
...
def get_tagged_items(request):
    ...
    return render(
        request, "playground/tagged_item.html", {"tagged_items": list(queryset)}
    )
...
def get_products_related_data(request):
    ...
    return render(
        request, "playground/product_related_data.html", {"products": products}
    )
...
def filter_using_q_with_not_operator(request):
    products = Product.objects.filter(~Q(unit_price__lt=20) & Q(inventory__lt=10))
    return render(request, "playground/products.html", {"products": list(products)})
